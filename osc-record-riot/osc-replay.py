...
addresses   = [line.split(',')[1] for line in lines[2:]]
messages   = [line.split(',')[2:] for line in lines[2:]]
...
def sendOSCMessage(idx):
    mess = messages[idx]
    liblo.send(target, addresses[idx], float(mess[0]), float(mess[0]), float(mess[0]))
#    if len(mess) == 2:
